...
const MODELS = [
  { id: 'yolo_hrca_seg', name: 'yolo_hrca_seg', desc: '当前生产模型', accuracy: '98.2%' },
];

export default function Dashboard() {
  const [isConfigured, setIsConfigured] = useState(false);
  const [lineAssignments, setLineAssignments] = useState<Record<number, number>>({});
  const [selectedModel, setSelectedModel] = useState('yolo_hrca_seg');
  ...
  useEffect(() => {
    ...
    api<Array<{ name: string; status: string }>>('/api/models')
      .then((models) => setSelectedModel(models.find((model) => model.status === 'active')?.name ?? 'yolo_hrca_seg'))
      .catch(() => undefined);
  }, []);
  ...
  const refreshLine = (lineId: number) => {
    setLineStats((previous) => ({ ...previous, [lineId]: { total: 1240, ng: 0 } }));
    notify(`产线 ${lineId} 的实时统计已重新同步`, 'info');
  };
  ...
  const generateReport = async () => {
    ...
    try {
      const payload = lines.map((line) => {
        const current = lineStats[line.id] ?? { total: 0, ng: 0 };
        const lineYield = current.total > 0 ? Number((((current.total - current.ng) / current.total) * 100).toFixed(1)) : line.yield;
        return { name: line.name, product: getAssignedProduct(line.id), yield: lineYield, total: current.total, ng: current.ng, status: line.status };
      });
      const result = await api<{ summary: string; downloadUrl: string }>('/api/reports/diagnostic', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ lines: payload }) });
              ...
              <div key={line.id} className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden group">
                <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => { setSelectedLine(line); setIsGridView(false); notify(`已切换到 ${line.name} 单屏监控`, 'info'); }} className="p-1.5 hover:bg-slate-50 rounded-lg transition-colors" title="单屏查看">
                ...
                <div className="p-4 bg-slate-50/50 flex items-center justify-between">
                  ...
                  <button onClick={() => refreshLine(line.id)} className="p-2 text-slate-400 hover:text-emerald-600 transition-colors" title="刷新产线统计">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setIsGridView(true)} className="text-xs text-emerald-600 font-bold hover:underline">管理全部</button>
            </div>
          ...
          <div className="bg-[#0F172A] p-6 rounded-3xl shadow-xl text-white relative overflow-hidden">
            <div className="relative z-10">
              ...
              <button disabled={reporting} onClick={generateReport} className="w-full py-3 bg-emerald-500 hover:bg-emerald-600 rounded-xl font-bold transition-all flex items-center justify-center gap-2 disabled:opacity-60">
                {reporting ? '正在生成报告…' : '生成诊断报告'} <ArrowRight className="w-4 h-4" />
              </button>
            </div>
